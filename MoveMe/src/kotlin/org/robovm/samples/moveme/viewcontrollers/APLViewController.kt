...
public class APLViewController : UIViewController() {

     {
        val moveMeView = APLMoveMeView();

        // Load the display strings.
        val resourcePath = NSBundle.getMainBundle().findResourcePath("DisplayStrings", "txt");
        var string = NSString.readFile(resourcePath, NSStringEncoding.UTF16BigEndian);

        if (string != null) {
            val displayStrings = string.split("\\r?\\n");
